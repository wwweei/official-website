...
<head>
    ...
    <script type="text/javascript">
        function addTradeNews(){
            var wWidth = 700;
            var wHeight = 900;
            var wTop = (window.screen.availHeight - 30 - wHeight) / 2;
            var wLeft = (window.screen.availWidth - 10 - wWidth) / 2; 
            window.open('add_tradeNews.jsp', "_blank", "scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes,height="+wHeight+"px, width="+wWidth+"px, top="+wTop+"px, left=" + wLeft + "px");
        }
...
<body>
    ...
    <table id="dg" class="easyui-datagrid" title="案例管理" style="width:100%;height:auto"
                    data-options="toolbar: '#toolbar',
                     ...
                     url:'/official-website/tradeNewsAction/allTradeNewsList.action', 
    ...
    <div id="toolbar">
            <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addTradeNews()">添加</a>
    ...
    function deleterow(){
        ...
        $.messager.confirm('确认删除？','确认删除？',function(r){
            if (r){
                ...
                $.ajax({
                	method: "get",
                	url: "/official-website/tradeNewsAction/deleteTradeNews.action?IDs=" + ids,
    ...
    function saverow(obj){
    	...
    	$.ajax({
        	method: "get",
        	url: "/official-website/tradeNewsAction/updateTradeNews.action?ID=" + row.ID + "&status=" + status,
        	success: function(data) {
                alert(data);
        	},
        	error: function(){
