...
<head>
    ...
    <script type="text/javascript">
        function addComNews(){
            ...
            window.open('add_comNews.jsp', "_blank", "scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes,height="+wHeight+"px, width="+wWidth+"px, top="+wTop+"px, left=" + wLeft + "px");
        }
...
<body>
    ...
    <table id="dg" class="easyui-datagrid" title="案例管理" style="width:100%;height:auto"
                    data-options="toolbar: '#toolbar',
                     ...
                     url:'/official-website/comNewsAction/allComNewsList.action', 
    ...
    <div id="toolbar">
            <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addComNews()">添加</a>
    ...
    function deleterow(){
        ...
        $.messager.confirm('确认删除？','确认删除？',function(r){
            if (r){
                ...
                $.ajax({
                	method: "get",
                	url: "/official-website/comNewsAction/deleteComNews.action?IDs=" + ids,
                	success: function(data) {
                		//每删除一行easyui的行索引就会刷新一次，正序删除时行索引变更会导致删除行错误，因此采用倒序，删除后面的行不会导致前面的索引变更
    ...
    function saverow(obj){
    	...
    	$.ajax({
        	method: "get",
        	url: "/official-website/comNewsAction/updateComNews.action?ID=" + row.ID + "&status=" + status,
        	success: function(data) {
                alert(data);
        	},
        	error: function(){
